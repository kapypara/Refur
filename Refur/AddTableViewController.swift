...
class AddTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        // MARK: unrelated debug
        let post = Database.Posts["test"].getData { error, snapshot in
            ...
            if let snapshot = snapshot?.value as? [String: Any]{
                //print(snapshot.keys.count)
                for (k, v) in snapshot {
                    
                    //guard let v = v as? [String: String] else { continue }
                    
                    print(k, v)
                    
                    //print(self.serviceProviders.last!.name)
